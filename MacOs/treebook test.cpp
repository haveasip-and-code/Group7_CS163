...
wxPanel *CreateDictionaryPage(wxBookCtrlBase *parent)
{
...
wxPanel *CreateFavouriteListPage(wxBookCtrlBase *parent)
{
    wxPanel *panel = new wxPanel(parent);
...
void CreateInitialPages(wxBookCtrlBase *parent)
{
    ...
    wxWindow *page = CreateAddPage(parent);
    parent->AddPage( page, ADD_NEW_WORD, false, GetIconIndex(parent) );
    
    page = CreateFavouriteListPage(parent);
    ...
    page = CreateDictionaryPage(parent);
...
wxWindow *CreatePage(wxBookCtrlBase *parent, const wxString&pageName)
{
    if ( pageName == DICTIONARY )
        return CreateDictionaryPage(parent);
    
    if ( pageName == FAVOURITE_LIST )
        return CreateFavouriteListPage(parent);
